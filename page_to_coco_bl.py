...
def pagexml_to_coco_split(pagexml_dir, output_dir, split_ratio=0.8, seed=42):
    ...
    def process_files(xml_files, subset_img_dir, subset_name):
        coco_output = {
            ...
            "categories": [
                {"id": 1, "name": "text_line"},
                {"id": 2, "name": "baseline"}  # Added baseline category
            ],
            "images": [],
            "annotations": []
        }
        ...
        for xml_file in tqdm(xml_files, desc=f"Processing {subset_name} PAGE-XML files"):
            ...
            for textline in root.findall('.//{*}TextLine'):
                ...
                if baseline is not None:
                    base_points_str = baseline.attrib.get('points', '')
                    base_points = [tuple(map(int, pt.split(','))) for pt in base_points_str.split()]
                    if len(base_points) >= 2:
                        offset = 1  # 1-pixel offset above/below

                        # Top and bottom edges
                        top_edge = [(x, y - offset) for x, y in base_points]
                        bottom_edge = [(x, y + offset) for x, y in reversed(base_points)]
                        baseline_polygon = top_edge + bottom_edge
                        flat_polygon = [coord for point in baseline_polygon for coord in point]

                        min_x = min(p[0] for p in baseline_polygon)
                        max_x = max(p[0] for p in baseline_polygon)
                        min_y = min(p[1] for p in baseline_polygon)
                        max_y = max(p[1] for p in baseline_polygon)

                        coco_output["annotations"].append({
                            "id": annotation_id,
                            "image_id": image_id,
                            "category_id": 2,  # baseline category
